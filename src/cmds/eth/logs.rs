...
//! `  <Event> @ <addr>  tx 0x…` using [`super::event_topics`] for
//! topic0 decoding. `data` >128 chars is truncated with byte count.
...
fn format_log_line(log: &LogEntry) -> String {
    ...
    let sig: String = match log.topics.first() {
        Some(t0) => match event_topics::lookup_hex(t0) {
